...
(function() { // private module pattern
  ...
  function normalizex(x)              { return Game.Math.normalize(x,   0, tower.w);                       }  
  function normalizeColumn(col)       { return Game.Math.normalize(col, 0, tower.cols);                    }  
  function x2col(x)                   { return Math.floor(normalizex(x)/COL_WIDTH);                        }  
  function y2row(y)                   { return Math.floor(y/ROW_HEIGHT);                                   }  
  function col2x(col)                 { return col * COL_WIDTH;                                            }  
  function row2y(row)                 { return row * ROW_HEIGHT;                                           }  
  function x2a(x)                     { return 360 * (normalizex(x)/tower.w);                              }  
  function tx(x, r)                   { return r * Math.sin((normalizex(x-camera.rx)/tower.w) *2*Math.PI); }  
  function ty(y)                      { return HEIGHT - HORIZON - (y - camera.ry);                         }  
  function nearColCenter(x,col,limit) { return limit > Math.abs(x - col2x(col + 0.5))/(COL_WIDTH/2);       }  
  function nearRowSurface(y,row)      { return y > (row2y(row+1) - ROW_SURFACE);                           }  
  ...
  var Player = Class.create({
    ...
    updateCollisionPoint: function(point) {
      ...
      if (point.cell.coin) {
        if (Game.Math.between(this.x + point.x, col2x(point.col+0.5) - COIN.W/2, col2x(point.col+0.5) + COIN.W/2) &&  
  ...
  var Renderer = Class.create({
    ...
    render: function(dt) {
      ...
      player.ry = Math.max(0, player.ry); 
